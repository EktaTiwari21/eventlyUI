...
const RegionSelectModal = ({ isOpen, onClose }: RegionSelectModalProps) => {
  ...
  // --- FIX: Filter the lists based on the searchQuery ---
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="relative bg-[#121212] w-full max-w-2xl rounded-lg shadow-lg text-white overflow-hidden border border-white/10" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-[#701615] transition-colors z-10">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>

        <div className="p-6">
          <div className="relative mb-4">
            <input type="text" placeholder={view === 'states' ? "Search for your state..." : "Search for your city..."} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-black/20 border-b-2 border-gray-600 py-2 pl-10 focus:border-red-500 focus:outline-none"/>
        ...
        <div className="p-6 border-t border-white/10 h-[60vh] overflow-y-auto">
          {view === 'states' ? (
            <div>
